...
/**
 * Retina screens can report a ratio of 3. This shader is fragment-heavy (up to
 * 30 swirl iterations per pixel), and at 3x it renders 9x the pixels of a
 * logical one. Capping at 2 is visually indistinguishable for a soft gradient
 * and roughly halves the work on 3x displays.
 */
...
export function AnimatedGradient({
  ...
  useEffect(() => {
    ...
    void main() {
      ...
      /** One full pass. `time` is a performance.now() timestamp. */
      ...
      // Only run while the element is actually on screen and the tab is
      // visible. Without this the shader burns GPU for every section of the
      ...
      const start = () => {
        ...
        running = true;
        // Rebase the clock so the animation resumes where it left off rather
        // than jumping ahead by however long it was paused.
        startTimeRef.current = performance.now() - elapsedWhenPaused * 1000;
        frameIdRef.current = requestAnimationFrame(animate);
      };
      ...
      const stop = () => {
        if (!running) return;
        running = false;
        elapsedWhenPaused = (performance.now() - startTimeRef.current) / 1000;
      ...
      // Paint one frame immediately so the section is never blank before the
